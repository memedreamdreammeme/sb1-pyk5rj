...
const SAMPLE_ACCOUNTS: TwitterAccount[] = Array.from({ length: 20 }, (_, i) => ({
  username: `user${i + 1}`,
...
function AccountRow({ account }: { account: TwitterAccount }) {
  return (
      ...
      <div className="flex items-center gap-4">
        <div className="grid grid-cols-5 gap-4 text-sm flex-1">
          <div className="overflow-hidden">
            ...
            <span className="ml-1 text-white/90 font-mono">{account.username}</span>
          </div>
          <div className="overflow-hidden">
            <span className="text-white/60">Password:</span>
            <span className="ml-1 text-white/90 font-mono">{account.password}</span>
          </div>
          <div className="overflow-hidden">
            <span className="text-white/60">Phone:</span>
            <span className="ml-1 text-white/90 font-mono">{account.phone}</span>
          </div>
          <div className="overflow-hidden">
            <span className="text-white/60">Email:</span>
            <span className="ml-1 text-white/90 font-mono">{account.email}</span>
          </div>
          <div className="overflow-hidden">
            <span className="text-white/60">2FA:</span>
            <span className="ml-1 text-white/90 font-mono">{account.twoFa}</span>
          </div>
        </div>
...
export function TwitterAccountsDialog() {
  const [activeTab, setActiveTab] = useState<'generate' | 'list'>('generate');
  const [accounts] = useState<TwitterAccount[]>(SAMPLE_ACCOUNTS);

  return (
    <DialogContent className="bg-white/[0.03] border-white/10 backdrop-blur-xl max-w-5xl">
      <DialogHeader>
        <DialogTitle className="text-white/90">Twitter Accounts</DialogTitle>
      </DialogHeader>
        ...
        {activeTab === 'generate' && (
          <div className="space-y-4">
            <p className="text-sm text-white/60">
              Generate up to 20 new Twitter accounts. Current: {accounts.length}/20
            </p>
